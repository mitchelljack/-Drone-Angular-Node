angular.module("demo").controller("jobController", function ($scope,$http) {
    $scope.jobTypes = [];
    $scope.Uavs = [];
    $scope.Pilots = [];
    $scope.searchText = "";
    ...
    $scope.updateFilter = function() {
        ...
        for(var i = 0; i < $scope.jobs.length; i++) {
            ...
            if($scope.statusFilter) {
                ...
                if($scope.statusFilter == "Pending")
                    filterStatus = "pending";
                else if($scope.statusFilter == "Active")
                    filterStatus = "active";
                else
                ...
                for(var j = 0; j < $scope.flights.length; j++)
                    if($scope.jobs[i].value.name==$scope.flights[j].job && $scope.flights[j].currentStatus==filterStatus) {
                        flag = true;
                        break;
                    }
                if(!flag)
            ...
            , pane: {
                ...
                , background: [{ // Track for Stand
                    ...
                    , series: [{
                        ...
                        , data: [{
                            color: $scope.filterJobs[index].value.color
                            , radius: '50%'
                            , innerRadius: '60%'
                            , y: $scope.filterJobs[index].value.percentage
                        }]
                    }]
                }, function callback() {
                    var chart = this
