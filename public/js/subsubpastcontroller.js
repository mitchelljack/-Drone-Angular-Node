angular.module("demo").controller("subsubpastController",function($scope,$routeParams,$http,$timeout){
    ...
    .success(function(data){
        ...
        $http.get("flight/fetch/gps/"+$scope.flight[0].value.assignedPBs.name+"/"+$scope.flight[0].id+"/"+$scope.flight[0].value.flight_starttime)
        .success(function(data){
            if(data.data != undefined && data.data.length>0){
                ...
                for (var i = 0; i < data.positions.length / 2; i++) {
                ...
                if(!window.mobileAndTabletcheck())
                    viewer.flyTo($scope.greenPin).then(function() {
                    });
                //var center = Cesium.Cartesian3.fromDegrees(data.data[0],data.data[1]);
