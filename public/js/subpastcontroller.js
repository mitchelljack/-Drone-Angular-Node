angular.module("demo").controller("subpastController", function ($scope,$routeParams,$http,$rootScope) {
    ...
    .success(function(data){
        if(data.rows){
            ...
            $scope.percentage = Math.round(data.rows[0].value.inactiveFlights)/((data.rows[0].value.inactiveFlights)+(data.rows[0].value.pendingFlights))*100;
